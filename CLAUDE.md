...
tests/               テスト
e2e/                 Playwright ビジュアルリグレッション
docs/goals/          初期実装計画と完了記録
```

## 開発ルール

- Claude Code は会話の応答を日本語で行う。
- TypeScript の型安全性を保つ。
...
`npm run e2e` は Playwright によるビジュアルリグレッションで、主要5画面（ダッシュボード/サブスク管理/CSV取込/レシート読取/実績）のスクリーンショットをベースラインと比較する。実行手順の詳細は [README.md](README.md) の「ビジュアルリグレッション (Playwright)」を参照。UI に意図的な変更を加えた場合は `npm run e2e:update` でベースラインを更新し、`e2e/__screenshots__/` の差分をコミットする。
